-- 000_init.sql — Combined idempotent initialization migration
-- Generated to allow a single SQL file to bootstrap the project's Supabase schema.
-- Safe to run multiple times; statements use IF NOT EXISTS / DROP ... IF EXISTS patterns.
-- Run in Supabase SQL editor or via psql / supabase CLI on a fresh database.
...
-- -----------------------------
-- Profiles and auth-related helpers
-- -----------------------------
...
-- -----------------------------
-- Rooms, membership, messages, read_receipts
-- -----------------------------
-- rooms
...
-- Safe to run on a fresh DB (no-op) and on a DB where the trigger
-- was installed after some users had already signed up.
-- -----------------------------
insert into public.profiles (id, username, display_name, avatar_url)
select
  u.id,
